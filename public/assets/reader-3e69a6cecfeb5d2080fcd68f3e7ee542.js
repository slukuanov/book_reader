...
(function() {
  var highlight, reader;

  highlight = {
    init: function() {
      this.initTextHighlighter();
      return this.highlighterManage();
    },
    highlighterModal: function(highligh, range) {
      var that;
      that = $(highligh);
      return that.popModal({
        ...
        onOkBut: function() {
          ...
          return window.saveHighlighter(that.html());
        },
        onCancelBut: function() {},
        onLoad: function() {
          return that.addClass('pre-highlight');
        },
        onClose: function() {
          var html;
    ...
    highlighterManage: function() {
      return $(document).on('click', 'span.highlighted, span.highlight', function(e) {
        var that;
        e.preventDefault();
        that = $(this);
        return that.popModal({
          ...
          onOkBut: function() {
            ...
            that.after(html);
            that.remove();
            return window.removeHighlighter(that.html());
          },
          onCancelBut: function() {},
          onLoad: function() {},
          onClose: function() {}
    ...
    initTextHighlighter: function() {
      var sandbox;
      sandbox = document.getElementById('bb-bookblock');
      return window.hltr = new TextHighlighter(sandbox, {
  ...
  reader = {
    ...
    changeFavButton: function(that, is_fav) {
      if (is_fav) {
        ...
        return that.find('span').text('DEL FROM FAVORITE');
      } else {
        ...
        return that.find('span').text('ADD TO FAVORITE');
      }
    },
    manageFavorite: function() {
      return $(document).on('click', '.favorite-manage', function(e) {
        var is_fav, mess, that, user_book_id;
        e.preventDefault();
        $('html, body').css("cursor", "wait");
        that = $(this);
        user_book_id = that.data('user-book-id');
        is_fav = that.hasClass('add-to-favorite');
        mess = is_fav ? 'added to' : 'removed from';
        return $.ajax({
          type: 'POST',
          ...
          success: function(data, textStatus, jqXHR) {
            ...
            return noty({
              text: 'Book ' + mess + ' your favorite list',
              type: 'success'
            });
          },
  ...
  $(function() {
    reader.init();
    highlight.init();
    window.setCurrentChapter = function(user_book_id, chapter_id) {
      return $.ajax({
        type: 'POST',
        url: '/book_readers/update_book',
        data: {
    ...
    window.saveHighlighter = function(text) {
      var chapter_id, user_book_id;
    ...
    return window.removeHighlighter = function(text) {
      var chapter_id, user_book_id;
      user_book_id = $('#menu-toc li.menu-toc-current').data('user-book-id');
      chapter_id = $('#menu-toc li.menu-toc-current').data('chapter-id');
      return $.ajax({
...
var Page = (function() {
	...
	function updateNavigation( isLastPage ) {
        var item = $('#menu-toc li.menu-toc-current').last();
        window.setCurrentChapter($(item).data('user-book-id'), $(item).data('chapter-id'));
...
(function (global) {
    ...
    function TextHighlighter(element, options) {
        ...
        this.options = defaults(options, {
            ...
            onAfterRemoveHighlight: function () { return true; },
    ...
    TextHighlighter.prototype.removeHighlights = function (element) {
        ...
        function mergeSiblingTextNodes(textNode, highlight) {
            ...
            self.options.onAfterRemoveHighlight(highlight)
        }
        ...
        function removeHighlight(highlight) {
            ...
            textNodes.forEach(function (node) {
                mergeSiblingTextNodes(node, highlight);
            });
        }
